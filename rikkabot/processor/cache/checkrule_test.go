	...
	var tests = []struct {
		...
		*message.Message
		...
		{ProcessRules: &control.ProcessRules{IsCallMe: true, EnableGroup: true}, // 0
			Message: &message.Message{Msgtype: message.MsgTypeText, RawContext: "/rikka hello",
		...
		{ProcessRules: &control.ProcessRules{IsCallMe: true, EnableGroup: true}, // 1
			Message: &message.Message{Msgtype: message.MsgTypeText, RawContext: "/rikka nihao",
		...
		{ProcessRules: &control.ProcessRules{IsCallMe: true}, // 3
			Message: &message.Message{Msgtype: message.MsgTypeText, RawContext: "safsdfas",
				GroupId: "", SenderId: "813466966", ReceiverId: "2788092443", IsGroup: false},
			want: false,
		},
