...
func init() {
	...
	rrPlugin.ProcessRules = &control.ProcessRules{IsAtMe: true, IsCallMe: true, EnableGroup: true, CheckWhiteGroup: true,
...
func (rrp *russianRoulettePlugin) waitChallengerAccept() bool {
	...
	_, b, _ := rrp.RecvMessage(&control.ProcessRules{IsAtMe: true, IsCallMe: true, CheckWhiteGroup: true, EnableGroup: true,
		ExecOrder: []string{"接受", "接受挑战", "accept", "ok", "go", "fine"}}, done)
